...
def bisect(size, file):
    ...
    g = gt.random_graph(size, lambda: np.random.poisson(4), directed=False)
...
#    v2 = ev[:,1]
    ...
    #pos = gt.arf_layout(g, max_iter=0)
    gt.graph_draw(g, pos=pos, output="bisection/{0}.pdf".format(
        file), vertex_color=[1, 1, 1, 0], vertex_fill_color=v_partition)
...
def getCommandLineArgs():
    ...
    parser.add_argument(
        '-n', help='size of the keys in Bits.', required=True, type=int)
